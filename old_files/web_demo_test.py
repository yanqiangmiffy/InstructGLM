...
def predict(input, max_length, top_p, temperature, history=None):
    ...
    for response, history in model.stream_chat(tokenizer, input, history, max_length=max_length, top_p=top_p,
                                               temperature=temperature,prompt_words=['指令','答案']):
        updates = []
        for query, response in history:
            updates.append(gr.update(visible=True, value="User：" + query))
            updates.append(gr.update(visible=True, value="InstructGLM：" + response))
        if len(updates) < MAX_BOXES:
            updates = updates + [gr.Textbox.update(visible=False)] * (MAX_BOXES - len(updates))
        yield [history] + updates


title = """<h1 align="center">🤖InstructGLM: Streaming Chatbot with Gradio🚀</h1>"""
